...
export default {
  async fetch(request, env, ctx) {
    ...
    const 管理员密码 = env.ADMIN || env.PASSWORD || env.TOKEN || env.KEY || env.UUID || env.uuid || 'default';
    const 加密秘钥 = env.KEY || '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改';
    const userIDMD5 = await MD5MD5(管理员密码 + 加密秘钥);
    const uuidRegex = /^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$/;
    const envUUID = env.UUID || env.uuid;
    const userID = (envUUID && uuidRegex.test(envUUID)) ? envUUID.toLowerCase() : [userIDMD5.slice(0,8),userIDMD5.slice(8,12),'4'+userIDMD5.slice(13,16),'8'+userIDMD5.slice(17,20),userIDMD5.slice(20)].join('-');
    const host = String(env.HOST || url.hostname).split(/[,，\s]+/).filter(Boolean)[0].toLowerCase().replace(/^https?:\/\//,'').split('/')[0].split(':')[0];

    if (env.PROXYIP) {
      const arr=await 整理成数组(String(env.PROXYIP));
      反代IP=arr[Math.floor(Math.random()*arr.length)]||'';
      启用反代兜底=false;
    } else {
      反代IP=((request.cf&&request.cf.colo)?request.cf.colo:'auto')+'.PrOxYIp.CmLiUsSsS.nEt';
      启用反代兜底=true;
    }

    if (upgradeHeader === 'websocket') {
      try { await 反代参数获取(url,userID); return await 处理WS请求(request,userID,url); }
      catch(e){ console.error('[VLESS WS] handler failed:',e?.message||e); return new Response('WebSocket error',{status:502}); }
    }
...
async function copySub(){try{await navigator.clipboard.writeText(document.getElementById("sub").value);showMsg("订阅地址已复制")}catch(e){showMsg("复制失败，请手动复制")}}load();
    ...
    if (url.pathname==='/admin/ADD.txt') {
      if (!env.KV) return new Response(JSON.stringify({error:'KV 未绑定'}),{status:500,headers:{'content-type':'application/json'}});
      if (request.method==='GET') return new Response(await env.KV.get('ADD.txt')||'',{headers:{'content-type':'text/plain;charset=utf-8','cache-control':'no-store'}});
      if (request.method==='POST') { const text=await request.text(); if(!text.trim()) return new Response(JSON.stringify({success:false,error:'IP列表不能为空'}),{status:400,headers:{'content-type':'application/json'}}); await env.KV.put('ADD.txt',text); return new Response(JSON.stringify({success:true}),{headers:{'content-type':'application/json'}}); }

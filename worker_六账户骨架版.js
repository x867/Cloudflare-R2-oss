...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (request.method === "POST" && !path.startsWith("/api/") && !path.startsWith("/admin/") && path !== "/sub") {
        const userID = await getSubscriptionUserID(env);
...
function handleWebSocket(request, ctx, uuid) {
  const url = new URL(request.url);
  return (async () => {
    await 反代参数获取(url, uuid);
    return await 处理WS请求(request, uuid, url);
  })();
}
...
// ===== 从 original/原始固定版_六账户额度统计版.js 搬运的运行逻辑 =====
